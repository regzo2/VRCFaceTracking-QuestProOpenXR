...
namespace Meta_OpenXR
{
    public class QuestProTrackingModule : ExtTrackingModule
    {
        ...
        private void UpdateMouthExpressions(ref UnifiedExpressionShape[] unifiedExpressions, ref float[] weights)
        {
            ...
            unifiedExpressions[(int)UnifiedExpressions.MouthUpperDeepenRight].Weight = Math.Max(0, expressions[(int)ExpressionFB.UPPER_LIP_RAISER_R] - expressions[(int)ExpressionFB.NOSE_WRINKLER_R]); // Workaround for upper lip up wierd tracking quirk.
